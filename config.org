...
;;; config.el --- Witchmacs config -*- lexical-binding: t; -*-
#+end_src
#+STARTUP: overview
#+TITLE: Witchmacs
#+LANGUAGE: en
#+OPTIONS: num:nil
The cutest Emacs 30+ distribution
...
* Quick install
#+BEGIN_SRC 
git clone https://github.com/derhfaust/Witchmacs ~/.emacs.d
#+END_SRC
...
~clangd~ as the language server backend for C and C++ autocompletion (via ~eglot~)

~jdtls~ (Eclipse JDT Language Server) for Java autocompletion (via ~eglot~) — Emacs will prompt to download/configure this automatically the first time you open a ~.java~ file and run ~M-x eglot~
...
doom-modeline is a modern, actively maintained modeline with native
support for eglot/flymake diagnostics, VC status, and icons. It
replaces spaceline + powerline, which are unmaintained and have a
malformed hook in the previous config.
...
#+BEGIN_SRC emacs-lisp
  (use-package dashboard
    ...
    (setq dashboard-startup-banner (expand-file-name "marivector.png" user-emacs-directory))
    ...
    (setq dashboard-navigator-buttons
      `((
         (nil "Witchmacs on github" "Open Witchmacs' github page on your browser"
              (lambda (&rest _) (browse-url "https://github.com/derhfaust/witchmacs")) default)
         (nil "Witchmacs crash course" "Open Witchmacs' introduction to Emacs"
    		  (lambda (&rest _) (find-file (expand-file-name "Witcheat.org" user-emacs-directory))) default)
         (nil "Update Witchmacs" "Get the latest Witchmacs update. Check out the github commits for changes!"
              (lambda (&rest _) (update-config)) default)
         )
        (
         (nil "Open scratch buffer" "Switch to the scratch buffer"
              (lambda (&rest _) (create-scratch-buffer)) default)
         (nil "Open config.org" "Open Witchmacs' configuration file for easy editing"
     		  (lambda (&rest _) (find-file (expand-file-name "config.org" user-emacs-directory))) default)
         ))))
#+END_SRC
*** Notes
...
#+END_SRC
** Initialize =vertico=, =orderless=, and =marginalia=

*** Description

Modern minibuffer completion stack for Emacs 28+. Vertico provides a
vertical completion UI, orderless allows flexible out-of-order fuzzy
matching, and marginalia adds helpful annotations (file sizes, command
descriptions, etc.) to completion candidates. This replaces ido and
ido-vertical-mode, which are showing their age compared to this stack.
...
;; Enable richer annotations and previews when using M-x, switch-buffer, etc.
...
*** Code
#+BEGIN_SRC emacs-lisp
  ;; irony replaced with eglot (built-in, Emacs 29+) + clangd
;; Install clangd: sudo apt install clangd  OR  sudo pacman -S clang
(use-package eglot
  :ensure nil
...
** Java
*** Description
Meghanada is no longer updated, use eglot instead (You can always disable if you don't need Java support).
*** Code

#+BEGIN_SRC emacs-lisp
;; meghanada is dead — replaced with eglot + eclipse.jdt.ls
;; On first use in a .java file, run M-x eglot and select jdtls.
(use-package eglot
  :ensure nil
