...
A quick note on completion: when you use commands like ~C-x C-f~ (find
file) or ~M-x~ (run command), you'll see a vertical list of matching
candidates (powered by Vertico). You don't need to type things in
order — typing space-separated fragments will match candidates
containing all of them in any order (powered by Orderless). For
example, typing ~con fil~ in ~M-x~ will match ~customize-file~.
